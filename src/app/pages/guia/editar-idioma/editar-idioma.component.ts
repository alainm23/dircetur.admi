...
export class EditarIdiomaComponent implements OnInit {
  ...
  ngOnInit() {
    this.subscribe_01 = this.database.getGuia_Idiomas2 ().subscribe ((response: any []) => {
  ...
  eliminar (item: any) {
    ...
    if (opcion == true) {
      this.database.removeGuia_Idiomas (item);
    } else {
  ...
  guardar (item: any) {
    ...
    if (name !== "") {
      ...
      this.database.updateGuia_Idiomas (item)
        .then (() => {
  ...
  guardar_nuevo () {
    if (this.nuevo != "") {
      ...
      this.database.addGuia_Idiomas (this.nuevo)
        .then (() => {
          this.nuevo = "";
          this.show_nuevo = false;
