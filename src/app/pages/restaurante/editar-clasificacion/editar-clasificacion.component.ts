...
export class EditarClasificacionComponent implements OnInit {
  ...
  ngOnInit() {
    this.subscribe_01 = this.database.getRestaurante_Clasificaciones2 ().subscribe ((response: any []) => {
  ...
  eliminar (item: any) {
    ...
    if (opcion == true) {
      this.database.removeRestaurante_Clasificaciones (item);
    } else {
  ...
  guardar (item: any) {
    ...
    if (name !== "") {
      ...
      this.database.updateRestaurante_Clasificaciones (item)
        .then (() => {
  ...
  guardar_nuevo () {
    if (this.nuevo != "") {
      ...
      this.database.addRestaurante_Clasificaciones (this.nuevo)
        .then (() => {
          this.nuevo = "";
          this.show_nuevo = false;
