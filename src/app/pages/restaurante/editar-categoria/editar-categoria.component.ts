...
export class EditarCategoriaComponent implements OnInit {
  ...
  ngOnInit() {
    this.subscribe_01 = this.database.getRestaurante_Categorias2 ().subscribe ((response: any []) => {
  ...
  eliminar (item: any) {
    ...
    if (opcion == true) {
      this.database.removeRestaurante_Categorias (item);
    } else {
  ...
  guardar (item: any) {
    ...
    if (name !== "") {
      ...
      this.database.updateRestaurante_Categorias (item)
        .then (() => {
  ...
  guardar_nuevo () {
    if (this.nuevo != "") {
      ...
      this.database.addRestaurante_Categorias (this.nuevo)
        .then (() => {
          this.nuevo = "";
          this.show_nuevo = false;
