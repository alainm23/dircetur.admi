...
export class ListaRestaurantesComponent implements OnInit {
  ...
  submit () {
    ...
    this.database.getRestauranteByDistritos (this.form.value.provincia.id, this.form.value.distrito.id)
      .subscribe ((data: any []) => {
        console.log (data);
        this.items = data;
      });
  ...
  detalle (item: any) {
    this.router.navigate(['/pages/restaurante/restaurante-detalle', item.id]);
  }
}
