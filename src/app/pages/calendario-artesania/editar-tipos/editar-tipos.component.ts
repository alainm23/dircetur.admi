...
export class EditarTiposComponent implements OnInit {
  ...
  ngOnInit() {
    this.subscribe_01 = this.database.getEventosArtesaniaTipos ().subscribe ((response: any []) => {
  ...
  eliminar (item: any) {
    ...
    if (opcion == true) {
      this.database.removeEventosArtesaniaTipos (item);
    } else {
  ...
  guardar (item: any) {
    ...
    if (nombre !== "") {
      ...
      this.database.updateEventosArtesaniaTipos (item)
        .then (() => {
  ...
  guardar_nuevo () {
    if (this.nuevo != "") {
      ...
      this.database.addEventosArtesaniaTipos (this.nuevo)
        .then (() => {
          this.nuevo = "";
          this.show_nuevo = false;
