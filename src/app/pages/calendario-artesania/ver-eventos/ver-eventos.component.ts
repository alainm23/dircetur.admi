...
export class VerEventosComponent implements OnInit {
  ...
  elimninar (item: any) {
    ...
    if (opcion == true) {
      console.log (item);
      this.database.deleteEventoArtesania (item.ref)
      .then (() => {
  ...
  editar (item: any) {
    this.router.navigate(['/pages/calendario-artesania/evento-detalle', item.ref.id]);
  }

  submit () {
    console.log (this.messelected);

    this.database.getEventosArtesaniaMes (this.messelected.value).subscribe ((data) => {
      console.log (data);
      this.items = data;
    });
  }
